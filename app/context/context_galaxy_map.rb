...
class ContextGalaxyMap < Context
  ...
  def create_map(args, systems)
    ...
    systems.each do |star_system|
      ...
      sprite = {
        x: location[0], y: location[1], w: 28, h: 28,
        path: 'sprites/PixelPlanets/shadowmap0.png',
        r: randr(0, 255), g: randr(0, 255), b: randr(0, 255),
  ...
  def check_system_select(args, systems)
    ...
    systems.each do |each_system|
      ...
      select_output << {
        x: each_system.x + 14, y: each_system.y - 4,
      ...
      select_output << {
        x: each_system.x - 2, y: each_system.y - 2,
        w: 32, h: 32,
